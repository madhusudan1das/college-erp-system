...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="h3 mb-0 text-gray-800">Questions: {{ $exam->title }} (Admin Master)</h2>
        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                @if ($exam->questions->isEmpty())
                    <div class="alert alert-info text-center py-4">No questions added yet. Use the form on the left to add questions or Import with AI.</div>
                @else
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start">
                                    ...
                                    <div class="d-flex gap-2 align-items-center">
                                        ...
                                        <form action="{{ route('admin.exams.questions.delete', $q->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this question?');">
                                            @csrf
                                            @method('DELETE')
...
$(document).ready(function() {
    ...
    $('.btn-edit-question').on('click', function() {
        ...
        $('#editQuestionForm').attr('action', `/admin/exams/questions/${id}`);
    ...
    $('#btnParseAI').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('admin.exams.questions.parse', $exam->id) }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#btnGenerateTopicAI').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('admin.exams.questions.generate', $exam->id) }}",
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
                topic: topic,
    ...
    $('#btnImportAI').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('admin.exams.questions.import', $exam->id) }}",
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
