    ...
    <div class="col-md-4 h-100 d-flex flex-column mb-4">
        <div class="card shadow h-100 mb-0">
            <div class="card-header bg-primary-grad text-white py-3">
                <h5 class="m-0 font-weight-bold"><i class="fas fa-users me-2"></i>Faculty Directory</h5>
            </div>
            <div class="card-body overflow-auto p-0" style="max-height: 500px;">
                <div class="list-group list-group-flush">
                    @foreach($faculty as $fac)
                        <a href="{{ route('student.chat', ['user_id' => $fac->user->id]) }}" class="list-group-item list-group-item-action d-flex align-items-center py-3 {{ $selectedUser && $selectedUser->id == $fac->user->id ? 'active bg-light text-dark font-weight-bold' : '' }}">
                            <div class="bg-primary text-white rounded-circle p-2 me-3 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px; min-width: 40px;">
                                <i class="fas fa-user-tie"></i>
                            </div>
                            <div class="w-100">
                                <div class="d-flex justify-content-between align-items-center">
                                    <h6 class="mb-0 fw-bold">Prof. {{ $fac->first_name }} {{ $fac->last_name }}</h6>
                                </div>
                                <span class="small text-muted">{{ $fac->department->name }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="card shadow h-100 d-flex flex-column mb-0">
            @if($selectedUser)
                ...
                <div class="card-footer bg-white border-top p-3">
                    <form action="{{ route('student.chat.send') }}" method="POST">
                        @csrf
            ...
            @else
                <div class="card-body d-flex flex-column align-items-center justify-content-center text-center py-5">
                    ...
                    <p class="text-muted" style="max-width: 400px;">Select a faculty member from the directory list on the left to start a direct private conversation.</p>
                </div>
            @endif
        </div>
    </div>
